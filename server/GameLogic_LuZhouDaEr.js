...
var GameLogic_LuZhouDaEr=function () {
    ...
    this.logic=function () {
        ...
        for(var roomId in rooms)
        {
            ...
            if(room.roomType!=RoomDefine.ROOM_TYPE_LUZHOUDAER)
            {
                continue;
            }
            ...
            switch(room.state)
            {
                ...
                case RoomDefine.ROOM_ACTION_STATE:
                {
                    ...
                    if(flowsNotify.flows.length>0)
                    {
                        ...
                        for(var uid in room_users) {
                            ...
                            if(roomUser.notifyFlowsCount()>0)//#1
                            {
                ...
                case RoomDefine.ROOM_DISMISS_STATE:
                {
                    ...
                    {
                        //
                        console.log("[超过5分钟,房间自动同意解散,roomId:"+room.roomId+"]");
    ...
    this.logFlowName=function (flowNo) {
        switch(flowNo) {
            ...
            case FlowDefine.START_AFTER_NO:
            {
                ...
                break
            case FlowDefine.START_AFTER_NO2:
            {
                console.log("[FlowDefine.START_AFTER_NO2]");
            }
                break
            case FlowDefine.TOUCH_21_CARD_NO:
            {
                console.log("[FlowDefine.TOUCH_21_CARD_NO]");
            }
                break;
            default:
            {
        ...
        switch(flowNo)
        {
            ...
            case FlowDefine.START_NO:
            {
                ...
                if(RoomDefine.isTest2==1)
                {
                    ...
                    if(room.lastZhuangUid==null)
                    {
                        if(zhuangJiaUser==null)
                        {
                            ...
                            tmpArray.sort(function(){ return 0.5 - Math.random() });
                            var u=tmpArray[0];
                            zhuangJiaUser=u;
                            zhuangJiaUser.roomUser.type=RoomDefine.ZHUANGJIA_TYPE;//设置为本次庄家
                ...
                if(RoomDefine.isTest2==1)
                {
                    ...
                    room.appendUserCard_test(2,1,2);
                    ...
                    room.appendUserCard_test(2,1,2);
                    room.appendUserCard_test(2,1,7);
                    room.appendUserCard_test(2,1,10);

                    room.appendUserCard_test(2,1,1);
                    room.appendUserCard_test(2,1,1);
                    room.appendUserCard_test(2,1,1);
                    ...
                    room.appendUserCard_test(2,1,6);
                    room.appendUserCard_test(2,0,6);
                    room.appendUserCard_test(2,0,6);

                    room.appendUserCard_test(2,0,8);
                    room.appendUserCard_test(2,0,8);
                    room.appendUserCard_test(2,0,8);

                    room.appendUserCard_test(2,1,8);
                    room.appendUserCard_test(2,1,9);
                    room.appendUserCard_test(2,1,10);
                ...
                //检查是否爆牌
                var tingPaiCards=zhuangJiaUser.roomUser.checkIsTingPai();
                if(tingPaiCards.length>0)
                {

                    var flow_bao=this.createFlow(room,1);
                    var action_bao=new Socket.Action();
                    action_bao.type=RoomDefine.ACTION_KE_BAOPAI;//0:发牌,1:提,2:跑,3:可吃,4:吃,5:可胡，6:胡，7:可碰,8:碰,9:等待出牌,10:出牌,11:隐藏选择按钮,12:取消超时等待
                    var groupCard_bao=new Socket.GroupCard();
                    action_bao.groupCards.push(groupCard_bao);
                    var tingPaiCount=tingPaiCards.length;
                    for(var i=0;i<tingPaiCount;i++)
                    {
                        var c=tingPaiCards[i];
                        groupCard_bao.cards.push(c);
                    }
                    action_bao.ack=1;
                    flow_bao.pushAction(zhuangJiaUser,action_bao);
                    flows.push(flow_bao);
                    ...
                    room.clearOperateQueue();
                    room.appendCanSelectedUserAction(zhuangJiaUser.uid,action_bao);
                    room.setAckState(RoomDefine.ROOM_WAIT_SELECTED_STATE);


                    console.log("["+room.roomId+",有爆牌,庄家:"+zhuangJiaUser.name+"]");
                }
                else{

                    this.logicFlow(FlowDefine.TOUCH_21_CARD_NO,room,flows);
            ...
            case FlowDefine.START_AFTER_NO:
            {
                ...
                for(var uid in users) {
                    ...
                    if (roomUser.type == RoomDefine.ZHUANGJIA_TYPE) {
                        ...
                        var action=roomUser.checkKeHu2(null,true,RoomDefine.ROOM_TOUCH_CARD_TYPE);
                        if(action!=null)
                        {
                            action.ack=1;
                            flow3.pushAction(roomUser,action);
                            hasHu=true;
                            roomUser.clientAck=0;//反馈标志
                            room.appendCanSelectedUserAction(roomUser.uid,action);
                        }
                ...
                if(hasHu)
                {
                    ...
                    this.logicFlow(FlowDefine.START_AFTER_NO2,room,flows);
                }
                ...
                for(var i=0;i<len;i++) {
                    ...
                    if(user.uid==tableTopObj.uid)
                    {
                        ...
                        if(tableTopObj.touchOrPut==RoomDefine.ROOM_PUT_CARD_TYPE)
                        {
                            ...
                            if(user.roomUser.type==RoomDefine.ZHUANGJIA_TYPE)
                            {
                                //庄家打出的第一张
                                if(room.zhuangjiaFirstCard==null)
                                {
                                    room.zhuangjiaFirstCard=tableTopObj.card;
                                }
               ...
               // else
                {
                    ...
                    //if(tableTopObj.touchOrPut==RoomDefine.ROOM_TOUCH_CARD_TYPE)
                    {
                        ...
                        for(var i=0;i<len;i++) {
                            ...
                            else if(room.zhuangjiaFirstCard.c_id==tableTopObj.card.c_id)
                            {
                                //检查黑摆或者3拢4坎
                                var action2=roomUser.checkKeHu2(null,true,RoomDefine.ROOM_TOUCH_CARD_TYPE);
                                if(action2!=null)
                                {
                                    action2.ack=1;
                                    var obj={};
            ...
            case FlowDefine.HU_NO:
            {
                ...
                for(var uid in users) {
                    ...
                    if(uid==oper.uid)
                    {
                        ...
                        var obj={};
                        if(huAction.type2==8||huAction.type2==9)//8:3拢4坎,9:黑摆
                        {
                            var shouzhongHu=roomUser.checkShouZhongHu();
                            var tableHu=roomUser.checkScore2();


                            obj.tableScore=tableHu;
                            obj.huScore=shouzhongHu;
                            obj.cards1=roomUser.sortRoomUserCard();
                            obj.cards2=roomUser.cards2;
                        ...
                        var hushu=obj.tableScore+obj.huScore;
                        roomUser.obj=obj;
                        ...
                        else if(hushu<=90)
                        {
                            ...
                            console.log("[玩家胡息超过90胡息!!!!!!!!!!!!!!!!!]");
                        }



                        roomUser.dunshu2=keValue;
                        roomUser.jiesuanHu=obj.tableScore+obj.huScore;
                        roomUser.dunshu=keValue;
                        ...
                        //天胡
                        if(huAction!=null&&huAction.type2==1)//0:,1:天胡,2:地胡,3:自摸胡,4:脱庄,5:闲加分 6:放炮,7无胡
                        {
                            roomUser.dunshu*=2;
                            console.log("[天胡囤x2]");
                        }
                        ...
                        {
                            if(room.zhuangjiaFirstCard!=null&&room.zhuangjiaFirstCard==room.lastTableObj.card)
                            {
                                //地胡
                                huAction.type2=2;
                                roomUser.dunshu*=2;
                        ...
                        //无胡
                        if(huAction!=null&&huAction.type2==7)//1:天胡,2:地胡,3:自摸胡 7:无胡,8:3拢4坎,9:黑摆(发完牌手中无红牌)
                        {
                            console.log("[无胡]");
                        }
                        ...
                        for(var i=0;i<cLen;i++)
                        {
                            ...
                            else if(size==3)
                            {
                                ...
                                else if(a1.type==a2.type&&a1.type==a3.type)
                                {

                                    if(this.isOneTwoThreeOrTwoSevenTen(arr)==0)
                                    {
                                        isAllHasHuXi=false;
                                        break;
                                    }
                        ...
                        if(isAllHasHuXi)//昆胡 所有牌型都有胡息
                        {

                            huAction.type3.push(9);
                        ...
                        //水上漂 11
                        if(room.lastTableObj.card.isShuiShangPiao!=undefined&&room.lastTableObj.card.isShuiShangPiao==1)
                        {
                            huAction.type3.push(11);
                            roomUser.dunshu*=2;
                            console.log("[水上漂x2]");
                        }
                        ...
                        for(var uid2 in users) {
                            var user2 = users[uid2];
                            var roomUser2 = user2.roomUser;
                            if(user==user2)
                            {
                                continue;
                            }
                            if(!roomUser2.canPengAndChi)
                            {
                                huAction.type3.push(13);
                                roomUser.dunshu*=2;
                                console.log("[杀爆x2]");
                        ...
                        //梗 14
                        var gengValue=parseInt(hushu);
                        if(gengValue>=10&&(gengValue%10)==0)
                        {
                            huAction.type3.push(14);
                            roomUser.dunshu*=2;
                            console.log("[梗x2]");
                        ...
                        if(room.gui==1)
                        {
                            var fourArr=roomUser.getFourGroup2();
                            if(fourArr.length>0)
                            {


                                var a=fourArr.length;
                                huAction.type3.push(a+1000);

                                roomUser.dunshu*=a;
                                console.log("[胡的玩家 归x"+fourArr.length+"]");
                            }
                        ...
                        //检查是否封顶
                        if(roomUser.dunshu>room.fengding)
                        {
                            console.log("[胡玩家超出封顶:"+roomUser.dunshu+",顶:"+room.fengding+"]");
                            roomUser.dunshu=room.fengding;
                ...
                for(var uid in users) {
                    ...
                    if (uid == oper.uid) {
                        ...
                        //放炮包赔
                        if(room.fangpaobaopei==1&&huAction!=null&&(huAction.type2==6||huAction.type2==2))//放炮或者地胡
                        {
                            //放炮者
                            if(room.tableTopObj.uid==uid)
                            {

                                roomUser.score1-=(myDunShu*2);
                                roomUser.dunshu=-(myDunShu*2);
            ...
            case FlowDefine.TOUCH_CARD:
            {
                ...
                if(nextCard==null)
                {
                    ...
                    for(var uid in users) {
                        ...
                        var result=roomUser.checkIsTingPai();
                        if(result.length>0)
                        {
                            var shouzhongHu=roomUser.checkShouZhongHu();
                            var tableHu=roomUser.checkScore2();

                            if((shouzhongHu+tableHu)>=10)
                            {
                                chaJiaoCount++;
                                user.chajiaoScore=(shouzhongHu+tableHu);
                            }
                            else{
                                console.log("[听牌但不满足查叫10胡,"+user.name+"]");
                            }
                    ...
                    if(chaJiaoCount>0&&chaJiaoCount<3)
                    {
            ...
            case FlowDefine.FORCE_ACTION_AFTER_OPER:
            {
                ...
                for(var uid in users) {
                    ...
                    if(tianHu!=null&&(tianHu.type2==8||tianHu.type2==9))//8:3拢4坎,9:黑摆
                    {
                        if(roomUser.type==RoomDefine.ZHUANGJIA_TYPE)
                        {
                            //庄家黑摆或者3拢4坎 过后流程
                            this.logicFlow(FlowDefine.START_AFTER_NO2,room,flows);
                            console.log("[庄家黑摆或3拢4坎后,超时间后流程]");
                            return;
                        }
                    }
    ...
    this.isOneTwoThreeOrTwoSevenTen=function (arr) {
...
module.exports = GameLogic_LuZhouDaEr;